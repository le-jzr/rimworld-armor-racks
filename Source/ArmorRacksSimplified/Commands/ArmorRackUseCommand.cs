...
namespace ArmorRacksSimplified.Commands
{
    public class ArmorRackUseCommand : Command
    {
        ...
        public override void ProcessInput(Event ev)
        {
            ...
            if (this.armorRack.IsForbidden(this.pawn.Faction))
                pawn.jobs.TryTakeOrderedJob(JobMaker.MakeJob(ArmorRacksJobDefOf.ArmorRacksSimplified_JobTransferToRack, armorRack), JobTag.Misc);
            else
                pawn.jobs.TryTakeOrderedJob(JobMaker.MakeJob(ArmorRacksJobDefOf.ArmorRacksSimplified_JobWearRack, armorRack), JobTag.Misc);
        }
    }
}
